...
- **The speaker box.** The page colours only what is certain, a tag, a name in the paragraph or your own pin; a turn is a guess and stays grey. Hover a grey line, or rest the cursor in one, and the box offers the cast as chips with the guess first; a click pins the speaker as a hidden `%% Name %%` comment, Ctrl+Z takes it back, and *Dialogue: tag the speaker* opens the box for the keyboard.
- **Voices on the manuscript page.** A toggle stripes each spoken paragraph in its speaker's colour, grey when nobody is sure; the page's hover box offers the same chips, `v` puts the keyboard on them, and a pin lands in the note as the hidden comment.
