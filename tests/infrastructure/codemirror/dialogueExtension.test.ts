...
const ROSTER = { "book/": [{ id: "m", name: "Mara", aliases: [], colour: "#111111", accent: [], accentNever: ["yes"] }, { id: "t", name: "Tomas", aliases: [], colour: "#222222", accent: ["aye", "ye'll"], accentNever: ["my", "yes"] }] };
...
describe("dialogueExtension", () => {
  ...
  it("tints speech and whole-paragraph thoughts and dims the narration under the dialogue lens", () => {
    h = mount(DOC, ext("book/ch1.md"), { lens: "dialogue" });
  ...
  it("leaves narration alone when told to", () => {
    h = mount(DOC, ext("book/ch1.md"), { lens: "dialogue", dialogue: { ...DEFAULT_SETTINGS.dialogue, dimNarration: false } });
...
describe("dialogueExtension — speakers", () => {
  ...
  it("colours a pinned line in the pinned speaker's colour, and a pin for a name with no note gets a colour of its own", () => {
    ...
    h = mount(doc, ext("book/ch1.md", {}, ROSTER), { lens: "dialogue" });
  ...
  it("tints speech in the speaker's colour, grey when nobody can be pinned, and says who on hover", () => {
    h = mount(EXCHANGE, ext("book/ch1.md", {}, ROSTER), { lens: "dialogue" });
  ...
  it("one colour and no hover without a cast, or when speaker colours are off", () => {
    h = mount(EXCHANGE, ext("book/ch1.md"), { lens: "dialogue" });
    expect(styles(h, "czm-speech")).toEqual([null, null, null]);
    expect(allFindings(h.view)).toEqual([]);
    h.destroy();
    h = mount(EXCHANGE, ext("book/ch1.md", {}, ROSTER), { lens: "dialogue", dialogue: { ...DEFAULT_SETTINGS.dialogue, speakerColours: false } });
    expect(styles(h, "czm-speech")).toEqual([null, null, null]);
  });

  it("uses the project's cast inside the project and follows a cast change", () => {
    const rosters = { ...ROSTER, "other/": [{ id: "i", name: "Ilse", aliases: [], colour: "#333333", accent: [], accentNever: [] }] };
    h = mount("“Go,” Ilse said.", ext("other/ch1.md", {}, rosters), { lens: "dialogue" });
    expect(styles(h, "czm-speech")).toEqual(["--czm-speech: #333333"]);
    expect(rosterFor(rosters, "other/ch1.md")[0]!.name).toBe("Ilse");
    expect(rosterFor(rosters, "book/ch1.md")[0]!.name).toBe("Mara");
    // Outside every project there is no cast: one colour, nothing to pin.
    expect(rosterFor(rosters, "elsewhere.md")).toEqual([]);
...
describe("dialogueExtension — accents", () => {
  ...
  it("marks a speaker's own accent words inside their speech only: green for uses, red for never", () => {
    h = mount(SCENE, ext("book/ch1.md", {}, ROSTER), { lens: "accents" });
  ...
  it("marks nothing under the dialogue lens", () => {
    h = mount(SCENE, ext("book/ch1.md", {}, ROSTER), { lens: "dialogue" });
...
describe("dialogueExtension — the lens's hands", () => {
  ...
  it("offers to take a marked word out of the character note, and names the certain speaker at the cursor", () => {
    ...
    h = mount(SCENE, [conventionsFacet.of({}), rostersFacet.of(ROSTER), dialogueExtension(() => "book/ch1.md", { editAccent: (s, list, term, add) => { edits.push([s.name, list, term, add]); } })], { lens: "accents" });
