...
describe("ManuscriptView", () => {
  ...
  it("lays the story over the page when asked: cast lines, scene cast in the pane, contradictions in the gutter", async () => {
    ...
    expect(blocks[2]!.classList.contains("is-active")).toBe(true);
    click(blocks[4]!);
    expect(v.contentEl.querySelector<HTMLElement>(".czm-ms-side-para .czm-ms-cm-badge.is-plant")!.textContent).toBe("plant");
  });
  ...
  it("marks echoes in the gutter on their own toggle, without the story", async () => {
    ...
    click(v.contentEl.querySelectorAll<HTMLElement>(".czm-ms-tool")[4]!);
    await tick();
