...
The head holds the framework dropdown (Truby, Generic, or yours if the file carries one), a search that dims everything but the matching cards, one line saying what is on the board (*3 stories · 12 cards · 1 unfiled*), **Fit**, the keyboard help, the **⋯** menu (the new note, the add, the new story, fit, the keys and the schema, each naming its command), and the jumps to the other panels. The sliders icon folds the side column, docked to the right: the group dropdown for new cards, **Add note…**, **New note…**, **New story…** and **Copy schema**, then three folded sections whose headers show their value: which layers are shown, a colour per group, and the tag prefix. A section you open stays open, and one you fold stays folded, across sessions; the map and the threads remember theirs the same way.
