...
- **Order** is manuscript order, the same the [story map](/guide/story-map), [plot grid](/guide/plot-grid) and [threads](/guide/story-threads) use: notes by path, unless `story-order: 3` in a note's front matter says otherwise.
...
- **Reading time** sits in the toolbar next to the word count (*4 h 5 min to read*) and beside each section's title. It is an estimate at an average adult pace, 250 words a minute; set your own under Settings → Manuscript → Reading speed.
...
| Up / Down, Enter | comment list | move; go to that paragraph on the page, the editor following |
| Shift + Enter | comment list | into the editor at that comment |
...
- **The plugin's notes**: `Story map.md`, `Story threads.md`, the writing log, an exported manuscript, the plot grid's export and snapshots.
...
Obsidian's `%% comments %%` are the right place for notes to self: they live in the text, sync with it, and never count as words. Reading view hides them; the manuscript page keeps them beside the text. One tag is the plugin's own: `%% REF: [[Card]] %%` links a passage to a card on the [writer board](/guide/writer#uses-and-lines), stays off the page and out of the export, and is counted by the board as a use.
...
- **The pane** (the speech-bubble toggle in the toolbar) has two parts. *This paragraph* shows the comments of the selected paragraph and one field: type `CHECK: was it a coat in chapter three?` and press **Enter**, and ` %% CHECK: was it a coat in chapter three? %%` lands at the end of that paragraph in the note, through the open editor if there is one. The tag is read from the prefix and shown as a chip while you type; no prefix means an untagged comment; Shift+Enter makes a new line. The chips under the field are a legend, and a click drops that prefix in. Press **c** on the page to jump to the field. *All comments* is one list of every comment and highlight of the manuscript in reading order, grouped under the chapter each is in, whose header stays put while its rows scroll by and carries the count; each row shows its tag as a bar down its edge and, in italics, the line of prose it hangs on; the dropdown filters by tag. *This paragraph* is pinned above the list and does not scroll with it. A click or Enter on a row takes the page to that paragraph, the editor following; a double click or Shift+Enter goes into the editor at the comment.
- A comment on a line of its own (`%%` … `%%` as a block) belongs to the paragraph before it.
- **Tags** colour a comment by the word it opens with. An uppercase word and a colon, nothing else. The defaults are `TODO` (something still to write), `FIX` (prose known to be wrong), `CHECK` (continuity, facts, names), `IDEA` (a possibility) and `CUT` (kept only until you are sure); add your own under Settings → Manuscript → Tags. **Resolve** a comment with the check mark at the end of its row (or `r` on the row): a `✓` goes in at the end of the comment, `%% TODO: the lamp ✓ %%`, so the tag and its colour survive, the row dims, the count says how many are still open, and the same mark reopens it. Delete the comment when it has nothing more to say.
...
- **Story** (the people toggle) lays the [story map](/guide/story-map) over the page: a cast line under each section title, everyone in it in the map's colours, most mentioned first; *In this scene* in the pane for the selected paragraph; a click on a name opens its note. A name the map found without a note (a *candidate*, dashed) opens a menu instead: character, place, item, faction or event makes the typed note in the project's folder for that kind, and *not a name* adds it to `story-ignore`, the same exits the map offers. Where the [threads](/guide/story-threads) view found two scenes disagreeing on a fact, a red diamond sits in the gutter of both, the hover box and the pane say what clashes, and a click on the row takes the page to the other scene (a double click, into its editor). This toggle builds the map on every refresh, so it is off by default. With it on, a [directed thread](/guide/story-threads#directed-threads)'s anchored plant is an accent-coloured mark in the gutter at its sentence, pointing on, and a payoff or reversal one pointing back, each naming the other end; a click takes the page there.
- **Voices** (the quote toggle) tints each sentence of speech in its speaker's colour, grey when nobody is sure, and dims the narration, exactly as the [dialogue lens](/guide/lenses#who-is-speaking) does in the editor, from the same cast and rules. Hover a sentence, or select the paragraph and press `v`, and the box offers the cast as chips: a click, or the arrows and Enter, pins the speaker as a hidden `%% Name %%` comment right before that sentence in the note; *Not speech* for quotes that are not dialogue, *Unpin* to take a pin back. This is the proof-reading pass: read the book with the talk lit and pin as you go. Off by default.
- **Gauge marks** (the activity toggle) put the [plot grid](/guide/plot-grid#the-value-gauge)'s value gauge in the gutter at each scene's heading: pipes for the scene's word on a graded thread's scale, red below neutral and green above, the word itself, and *inversion* where the running total flips, in italics when no quote marks the turn. Hover for the total. Builds the threads, so it is off by default; a setting under Manuscript keeps it.
- **Echoes** (the repeat toggle) marks what the [echo finder](/guide/story-threads#echoes) heard: a purple dot in the gutter at every sentence holding a phrase that recurs elsewhere in the book, its title naming the nearest other place the words occur; a click takes the page there, and the pane and the hover box list the echoes of the selected paragraph. Off by default for the same reason as *Story*.
