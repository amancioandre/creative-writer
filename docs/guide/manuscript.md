...
- **Voices** (the quote toggle) stripes each spoken paragraph in its speaker's colour, grey when nobody is sure, from the same cast and rules as the [dialogue lens](/guide/lenses#who-is-speaking). Hover a striped paragraph, or select it and press `v`, and the box offers the cast as chips: a click, or the arrows and Enter, pins the speaker as a hidden `%% Name %%` comment in the note; *Not speech* for quotes that are not dialogue, *Unpin* to take a pin back. This is the proof-reading pass: read the book with the talk lit and pin as you go. Off by default.
