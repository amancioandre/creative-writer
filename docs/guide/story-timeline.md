...
*Who is where.* Every scene of the project in reading order down the side, the cast across the top, a dot where someone is present. Open it from the ribbon (the chart icon), with **Open story timeline**, or with the **Timeline** button in the story map's panel.
...
The timeline is a projection of the same graph the [story map](/guide/story-map) builds — same notes, same mention extraction, same `Story map.md` readings — so the two never disagree, and both refresh together when you edit.
