...
- **A summary under the name.** A comment line right under a heading in `Story threads.md` is the column's summary, what the arc is or what the theme argues, shown under the name in three lines at most and whole on hover. **Summary…** in the column's menu, or the side column's *Column* section, writes it; an emptied field takes the line out.
