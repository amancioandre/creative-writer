...
- README, under **Privacy**, one paragraph: *After you install the plugin, and once after each minor update, it shows a short note about what changed with a link to a feedback form hosted on Tally. The note is built into the plugin; nothing is fetched and nothing is sent unless you open the link, and the link carries nothing about you or your vault. Turn it off under Settings → Where it runs → Release notes.* The same paragraph goes in `reference/settings.md` and `reference/faq.md` ("Why did a window open after the update?").
