...
| `Story map.md` | Inside each project folder | Model readings per scene (relationships, references, events, with a hash of the prose read), and where you pinned nodes by hand | **Yes** — it is a Markdown note, and every sync method carries Markdown. |
| Entity notes, `## Relationships` lines, `story-ignore`, `aliases` | Your notes | Every decision you make in the story map, including nodes and relationships you draw | Yes — they are your notes. |
...
- **`Story map.md` conflicts** are rare — it changes when you run a reading or pin a node — and harmless: the loser's readings are simply re-run on the next *Read project*, because unchanged scenes are recognised by hash, and a lost pin is one drag away.
