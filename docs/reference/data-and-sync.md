...
| Template notes | `Creative Writer/Templates/` (Settings → Stories and goals) | Grid templates: `# Columns` with `##` headings, then acts, chapters and scenes with loglines and `beat:` comments; the built-ins ship in the plugin and are never fetched | **Yes** — and they are prose you can edit. |
| `Outline.md` | Inside each project folder | The plot grid's plan before the chapters exist: `#` act, `##` chapter, `###` scene, a comment under a scene as its logline; marked `creative-writer-outline-built` once built and kept as the record | **Yes** — and it is prose you can edit. |
