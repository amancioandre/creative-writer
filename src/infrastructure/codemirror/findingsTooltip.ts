...
/** What the hover card needs: a range, a kind word for the label and class, and the note. A style `Finding` is one. */
export interface HoverFinding {
...
export function allFindings(view: EditorView): HoverFinding[] {
...
export function findingAt(findings: readonly HoverFinding[], pos: number): HoverFinding | null {
...
export function tooltipFor(f: HoverFinding): Tooltip {
  return {
    pos: f.from,
    end: f.to,
    above: true,
